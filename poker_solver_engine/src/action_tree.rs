...
#[cfg(feature = "bincode")]
impl<'de> BorrowDecode<'de, ()> for ActionTreeNode {
    fn borrow_decode<D: BorrowDecoder<'de, Context = ()>>(decoder: &mut D) -> Result<Self, DecodeError> {
        Ok(Self {
            player: BorrowDecode::borrow_decode(decoder)?,
            board_state: BorrowDecode::borrow_decode(decoder)?,
            amount: BorrowDecode::borrow_decode(decoder)?,
            actions: BorrowDecode::borrow_decode(decoder)?,
            children: BorrowDecode::borrow_decode(decoder)?,
        })
    }
}
...
impl ActionTree {
    ...
    fn build_tree(&mut self) {
        ...
        *root = ActionTreeNode::default();
        ...
        self.build_tree_recursive(BuildTreeInfo::new(self.config.effective_stack));
    }

    /// Recursively builds the action tree.
    fn build_tree_recursive(&self, info: BuildTreeInfo) {
        let mut root = self.root.lock();
        *root = ActionTreeNode::default();
        root.board_state = self.config.initial_state;
        self.build_tree_recursive_helper(&mut root, info);
    }

    /// Recursive helper function for building the tree.
    fn build_tree_recursive_helper(&self, node: &mut ActionTreeNode, info: BuildTreeInfo) {
        if node.is_terminal() {
            // do nothing
            ...
            self.build_tree_recursive_helper(
